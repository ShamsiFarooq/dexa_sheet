...
class SheetProvider extends ChangeNotifier {
  ...
  try {
    ...
    notifyListeners();
    final repo = loadUseCase.repository;
    debugPrint('Dexa DEBUG create: repo=${repo.runtimeType}');
    final id = await repo.createSheet(name: name);
    await load(id);
  } catch (e, st) {
    debugPrint('Create new sheet failed: $e\n$st');
  } finally {
    isLoading = false;
    WidgetsBinding.instance.addPostFrameCallback((_) => notifyListeners());
  }
...
Future<void> save() async {
  try {
    debugPrint('Dexa DEBUG save: repo=${saveUseCase.repository.runtimeType}');
    _sheet.lastModified = DateTime.now();
    await saveUseCase(_sheet);
    debugPrint('Dexa DEBUG save OK for id=${_sheet.id}');
  } catch (e, st) {
  ...
  void addRow({int? index}) {
    final cols = _sheet.data.isNotEmpty ? _sheet.data[0].length : 0;
    final newRow = List.generate(cols, (_) => '');
    if (index == null || index >= _sheet.data.length - 1) {
      _sheet.data.add(newRow);
    } else {
      _sheet.data.insert(index + 1, newRow);
    }
    _sheet.rows = _sheet.data.length;
    _sheet.lastModified = DateTime.now();
    notifyListeners();
    _debouncedSave();
  }
  ...
  void addColumn({int? index}) {
    for (var row in _sheet.data) {
      if (index == null || index >= row.length - 1) {
        row.add('');
      } else {
        row.insert(index + 1, '');
      }
    }
    _sheet.cols = _sheet.data[0].length;
    _sheet.lastModified = DateTime.now();
    notifyListeners();
    _debouncedSave();
  }
  ...
  void removeColumn(int index) {
    if (_sheet.data.isEmpty || _sheet.data[0].length <= 1) return;
    for (var row in _sheet.data) {
      row.removeAt(index);
    }
    _sheet.cols = _sheet.data[0].length;
  ...
  String exportCsv() => const ListToCsvConverter().convert(_sheet.data);
